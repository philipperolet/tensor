...
def plot_new_loss_xp(filename):
    ...
    for j, (dataset, ds_size) in enumerate(product(['mnist', 'cifar'], ['normal', 'full'])):
        ...
        print([f"{d['duration']} - {d['param_combination']['activation_function']} - {dataset} {ds_size}" for d in data_slice])
        ...
        for i, d in product(range(len(data_slice) - 1), range(2)):
            ...
            plt.errorbar(
                ...
                label=data_slice[i + d]['param_combination']['activation_function'],
                capsize=5,
            )
            plt.legend()
    plt.show()
