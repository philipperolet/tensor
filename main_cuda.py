...
data = dict(
    training_input=train_data.cuda(),
    training_target=train_target.cuda(),
    test_input=test_data.cuda(),
...
def compute_hidden_layer_test_error(hidden_layer_size):
    return CustomNetTrainer(CustomNet(hidden_layer_size).cuda(), data, parameters, loss=mods.CrossEntropyLoss().cuda()).train()
